...
public class InvoiceOutputHandler extends DefaultCrudHandler{
	
	@Override
	public Object execute(ExecutionEvent event) throws ExecutionException {
		IWorkbenchWindow window = HandlerUtil.getActiveWorkbenchWindow(event);
		fireCrudService(window,"invoiceOutput");
		IPerspectiveRegistry registry = window.getWorkbench().getPerspectiveRegistry();
		IWorkbenchPage page = window.getActivePage();
		page.closeAllEditors(true);
		page.setPerspective(registry.findPerspectiveWithId(InvoiceOutputPerspective.ID));
		return null;
	}
}
